...
@Injectable()
export class BlogImageTrackerService
  implements BlogImageTrackerServiceInterface
{
  constructor(
  ...
  async trackImagesInBlog(blogId: number, content: string): Promise<void> {
    try {
      ...
      if (removedUrls.length > 0) {
        ...
        console.log(
          `Deactivated ${removedUrls.length} removed images for blog ${blogId}`
        );
      }
      ...
      for (const imageUrl of imageUrls) {
        ...
        if (!existingImage) {
          ...
          console.log(
            `Added new image tracking for blog ${blogId}: ${imageUrl}`
          );
      ...
      console.log(
        `Tracking ${imageUrls.length} images for blog ${blogId}:`,
        imageUrls
      );
    } catch (error) {
  ...
  async cleanupOrphanedImages(): Promise<void> {
    try {
      ...
      console.log(
        `Found ${orphanedImages.length} orphaned images, cleaning up...`
      );
      ...
      for (const imageUrl of orphanedImages) {
        try {
          ...
          console.error(
            `Failed to delete orphaned image ${imageUrl}:`,
            deleteError
          );
        }
      }
