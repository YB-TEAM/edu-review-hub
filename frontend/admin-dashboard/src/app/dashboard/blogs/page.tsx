...
export default function BlogsPage() {
  ...
  const uniqueAuthors = blogs
    ...
    .filter(
      (author, index, arr) =>
        author.id && arr.findIndex((a) => a.id === author.id) === index
    )
    .filter(Boolean);
    ...
    try {
      switch (action) {
        case "approve":
          await Promise.all(
            selectedBlogs.map((id) =>
              updateBlog({ id, data: { status: "approved" as any } }).unwrap()
            )
          );
        ...
        case "draft":
          await Promise.all(
            selectedBlogs.map((id) =>
              updateBlog({ id, data: { status: "draft" as any } }).unwrap()
            )
          );
  ...
  const handleExport = async (format: string) => {
    try {
      const result = await exportBlogs({
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.success(
        `Import thành công: ${result.success} blog, thất bại: ${result.failed}`
      );
      refetch();
    } catch (error) {
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-300 mt-2">
            Quản lý nội dung blog và bài viết trong hệ thống
          </p>
        </div>
        <div className="mt-4 md:mt-0 flex space-x-2">
          <div className="relative">
            ...
            <label htmlFor="import-file">
              <Button
                variant="outline"
                className="cursor-pointer hover:bg-blue-50 hover:border-blue-200 hover:text-blue-600 transition-all duration-200 transform hover:scale-105"
              >
          ...
          <div className="relative group">
            <Button
              variant="outline"
              className="flex items-center hover:bg-green-50 hover:border-green-200 hover:text-green-600 transition-all duration-200 transform hover:scale-105"
            >
      ...
      {viewMode === "table" ? (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Tổng cộng {totalItems} blog</CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {currentBlogs.map((blog) => (
                    <tr
                      key={blog.id}
                      className="border-b hover:bg-gray-50 dark:hover:bg-gray-800 transition-all duration-200 ease-in-out group"
                    >
                      <td className="p-3">
                        <input
                          type="checkbox"
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              ...
                              setSelectedBlogs(
                                selectedBlogs.filter((id) => id !== blog.id)
                              );
                            }
                          }}
                      ...
                      <td className="p-3">
                        <div className="flex items-center space-x-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="font-medium text-gray-900 dark:text-white truncate group-hover:text-blue-600 transition-colors duration-200">
                              {blog.title}
                            </div>
                            ...
                            <div className="flex items-center space-x-2 mt-1">
                              {blog.tags.slice(0, 2).map((tag, index) => (
                                <Badge
                                  key={index}
                                  variant="outline"
                                  className="text-xs hover:bg-blue-50 hover:border-blue-200 transition-colors duration-200"
                                >
                                  {tag.name}
                                </Badge>
                              ))}
                      ...
                      <td className="p-3">
                        <select
                          ...
                          className={`border border-gray-300 dark:border-gray-600 rounded px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 dark:bg-gray-800 dark:text-white transition-all duration-200 hover:border-blue-300 dark:hover:border-blue-400 bg-white ${getStatusColor(
                            blog.status
                          )}`}
                        >
                          <option
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between mt-6">
                <div className="text-sm text-gray-600">
                  Hiển thị {(currentPage - 1) * blogsPerPage + 1}-
                  {Math.min(currentPage * blogsPerPage, totalItems)} trong tổng
                  số {totalItems} blog
                </div>
                <div className="flex space-x-2">
                  <Button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {currentBlogs.map((blog) => (
            <Card
              key={blog.id}
              className="hover:shadow-xl hover:-translate-y-1 transition-all duration-300 ease-in-out group border-0 shadow-md hover:border-blue-200 dark:hover:border-blue-700"
            >
              ...
              <CardHeader className="pb-3">
                ...
                <CardDescription className="line-clamp-2 group-hover:text-gray-700 dark:group-hover:text-gray-300 transition-colors duration-300">
                  {blog.excerpt}
                </CardDescription>
              </CardHeader>
              <CardContent className="pt-0">
                <div className="flex items-center space-x-2 mb-3">
                  ...
                  <span className="text-sm text-gray-600 dark:text-gray-400 group-hover:text-blue-600 transition-colors duration-300">
                    {blog.authorName || "Unknown"}
                  </span>
                </div>
                ...
                <div className="flex items-center space-x-2 mb-3">
                  {blog.tags.slice(0, 3).map((tag, index) => (
                    <Badge
                      key={index}
                      variant="outline"
                      className="text-xs hover:bg-blue-50 hover:border-blue-200 hover:text-blue-600 transition-all duration-300 transform hover:scale-105"
                    >
                      {tag.name}
                    </Badge>
                  ))}
